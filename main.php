...
$buttonString = "";
foreach ($r as $cat){
    $buttonString .= '<button class="btn btn-outline-light m-1 border border-dark" style="background-color: '.$cat["color"].'" data-color="'.$cat["color"].'" data-status="enabled" data-category="'.$cat["categoryID"].'" onclick="categoryFilterButtonClick(this)">'.$cat["name"].'</button>';
}
...
<head>
    ...
    <style type='text/css'>
        ...
        #activeFeedbackItems li { margin: 3px 3px 3px 0; padding: 1px; float: left; width: 100px; height: 90px; font-size: 1em; text-align: center; }
        #passiveFeedbackItems { list-style-type: none; margin: 0; padding: 0; width: 450px; }
        #passiveFeedbackItems li { margin: 3px 3px 3px 0; padding: 1px; float: left; width: 100px; height: 90px; font-size: 1em; text-align: center; }
    </style>
</head>
...
<div class="row">
    ...
    <div id="mainScreen" class="col-sm-10" hidden>
        <div class="container-fluid text-center mt-3 mb-3">
            <p class="h2 d-block">Feedback Editor </p>
            <u>Submission by <span id="studName"></span> (<i class="fas fa-envelope-open-text" data-toggle="tooltip" title="Open submission"></i>)</u>
